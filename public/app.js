...
var handleSignedInUser = function(user) {
  ...
  var docRef = db.collection("weeklyquiz").doc("week600");
  docRef.get().then((doc) => {
    if (doc.exists) {
        console.log("QuizWeek600: data:", doc.data());
    } else {
        // doc.data() will be undefined in this case
        console.log("No QuizWeek600!");
    }
}).catch((error) => {
    console.log("Error getting QuizWeek600:", error);
});
...
firebase.auth().onAuthStateChanged(function(user) {
  document.getElementById('check-this').addEventListener('click', function() {
    ...
    document.getElementById('is-new-user').textContent = userDispName + " Clicked";
  });
